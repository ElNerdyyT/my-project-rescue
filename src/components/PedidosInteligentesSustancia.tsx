...
const SUCURSALES = [
  ...
  // Agrega aquí tus otras sucursales si es necesario
];
...
const PedidosInteligentesSustancia = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // 1. OBTENER DATOS CON JOIN A LA TABLA MAESTRA
        ...
        // Aplanamos el resultado para un uso más fácil
        ...
        const sugerenciasCalculadas: SugerenciaAgrupada[] = Object.entries(datosAgrupados).map(([grupo, data]) => {
            ...
            const { venta, movs } = data as { venta: number; movs: number };
            ...
            return {
                ...
                costo_promedio: primerArticulo.costo_a, // Se puede mejorar a un promedio ponderado
                articulos_individuales: articulosDelGrupo
            };
        }).filter(Boolean).sort((a,b) => b.venta_periodo - a.venta_periodo);
  ...
  // La función de exportar a PDF y el resto del componente se adaptan para usar la nueva estructura.
  const handleExportPDF = () => {
    const doc = new jsPDF();
    // ... Lógica del PDF adaptada ...
    doc.save(`pedidos-agrupados-${selectedBranch}-${new Date().toISOString().split('T')[0]}.pdf`);
  };

  return (
    <div className="col-12">
      <div className="card">
         {/* Aquí va tu JSX para la UI: Título, botones, etc. Este no necesita grandes cambios */}
         {/* ... */}
                ...
                {sugerencias
                  ...
                  .map(sugerencia => (
                    <tr key={`${selectedBranch}-${sugerencia.id}`}>
                      ...
                      <td>
                        {/* El sistema de edición de pedidos sigue funcionando igual */}
