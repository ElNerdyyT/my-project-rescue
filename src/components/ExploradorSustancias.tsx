    ...
    <div class="container mx-auto p-2 sm:p-4 md:p-6">
      <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-4 sm:p-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <h2 class="text-2xl font-bold text-slate-800">Explorador de Sustancias</h2>
        </div>
        
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <input type="text" placeholder="Buscar por nombre de sustancia..." value={busqueda} onInput={e => setBusqueda(e.currentTarget.value)} class="md:col-span-2 form-input w-full px-4 py-2 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
          <select value={filtroStock} onChange={e => setFiltroStock(e.currentTarget.value)} class="form-select w-full px-4 py-2 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
        ...
        <div class="overflow-x-auto rounded-lg border border-slate-200">
          <table class="min-w-full divide-y divide-slate-200">
            ...
            <tbody class="bg-white divide-y divide-slate-200">
              ...
              {!loading && sustanciasFiltradas.map(sustancia => (
                <tr key={sustancia.nombre} onClick={() => handleAbrirModal(sustancia)} class="hover:bg-indigo-50 cursor-pointer transition-colors duration-150">
                  <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-slate-900">{sustancia.nombre}</div></td>
                  <td class="px-6 py-4 whitespace-nowrap text-right"><span class={`text-base font-bold ${sustancia.stockTotal > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{sustancia.stockTotal}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {modalState.isOpen && modalState.sustancia && (
        <div style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.7)', zIndex: 50, display: 'flex', justifyContent: 'center', alignItems: 'center'}} onClick={() => setModalState({ ...modalState, isOpen: false })}>
          <div class="bg-slate-100 rounded-xl shadow-2xl w-11/12 max-w-4xl max-h-[90vh] flex flex-col transition-transform duration-300" onClick={e => e.stopPropagation()}>
            <div class="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-white rounded-t-xl">
              <div>
                  <h3 class="text-xl font-bold text-slate-900">{modalState.sustancia.nombre}</h3>
                  <p class="text-sm text-slate-500">Stock Total: <span class="font-bold text-lg text-emerald-600">{modalState.sustancia.stockTotal}</span></p>
              </div>
            ...
            <div class="p-4 sm:p-6 overflow-y-auto">
              {modalState.isLoading ? (
                <div class="text-center p-8 text-slate-600">Cargando detalles...</div>
              ) : (
                <div class="space-y-6">
                  {modalState.detalles.map(producto => (
                    <div key={producto.cve_articulo_a} class="bg-white rounded-lg border border-slate-200 overflow-hidden shadow-sm">
                      ...
                      <div class="overflow-x-auto">
                        <table class="min-w-full"><thead class="bg-slate-100 text-xs text-slate-500"><tr><th class="text-left p-2 font-medium">Sucursal</th><th class="text-right p-2 font-medium">Cantidad</th><th class="text-right p-2 font-medium">Costo</th><th class="text-left p-2 font-medium">Últ. Modificación</th></tr></thead>
                          <tbody class="text-sm">
                            {producto.sucursales.map(s => (<tr key={s.nombre} class="border-t border-slate-100"><td class="p-2 font-medium text-slate-700">{s.nombre}</td><td class={`p-2 text-right font-semibold ${s.cantidad < 0 ? 'text-rose-600' : 'text-slate-800'}`}>{s.cantidad}</td><td class="p-2 text-right text-slate-600">{formatCurrency(s.costo)}</td><td class="p-2 text-slate-600">{s.fecha_mod}</td></tr>))}
                          </tbody>
                        </table>
                      </div>
                    </div>
                  ))}
                  {modalState.detalles.length === 0 && <p class="text-center text-slate-500 py-4">No se encontraron productos para esta sustancia.</p>}
                </div>
              )}
            </div>
            <div class="px-6 pt-4 pb-6 border-t border-slate-200 bg-white/50 backdrop-blur-sm rounded-b-xl">
                ...
                {modalState.relacionadas.length > 0 && (
                  <div class="mt-4">
                    <h4 class="font-bold text-sm mb-2 text-slate-700">Otras presentaciones:</h4>
                    <div class="flex flex-wrap gap-2">
                      {modalState.relacionadas.map(rel => (
                        <button key={rel.nombre} onClick={() => handleAbrirModal(rel)} class="bg-slate-200 text-slate-800 text-xs font-medium px-3 py-1 rounded-full hover:bg-slate-300 transition-colors">
